...
class Post(db.Model):
    ...
    def __repr__(self) -> str:
        """Return ID as string, for db operations."""
        return str(self.id)

    def show(self) -> dict[str, int | str]:
        """Return human readable representation as a dict."""
        return {'id': self.id,
                'userId': self.userId,
                'title': self.title,
                'body': self.body}
    ...
    def get(cls, post_id) -> tuple[dict[str, int | str], int]:
        """
        Perform GET request with post ID as parameter.
    ...
    def user_get(cls, user_id) -> tuple[(Response | dict[str, int | str]), int]:
        """
        Perform GET request with user ID as parameter.
        ...
        if not posts:
            return {'msg': f'No posts from user {user_id}'}, 404
    ...
    def update(cls, post_id) -> tuple[dict[str, dict[str, int | str]], int]:
        """
        Perform PUT request with post ID as parameter.
        Example url: http://127.0.0.1:5000/posts/104
        """
        ...
        post: Post = cls.query.get(post_id)
        if post is None:
            return {'msg': f'Post ID {post_id} not found.'}, 404
    ...
    def delete(cls, post_id) -> tuple[dict[str, str], int]:
        """
        Perform DELETE request with post ID as parameter.
        Example url: http://127.0.0.1:5000/posts/104
        """
        post: Post = cls.query.get(post_id)
        if post is None:
        ...
        db.session.commit()
        return {'msg': f'Post with ID {post_id} deleted.'}, 200
