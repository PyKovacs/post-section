...
def validate_input(request, action: str = 'add_post'):
    ...
    try:
        ...
        if not p_title or len(str(p_title)) > 80:
            return {'msg': 'Title value is missing or too long (max 80 chars).'}, 422

        # body
        p_body = request_json['body']
        if not p_body or len(str(p_body)) > 500:
            return {'msg': 'Body value is missing or too long (max 500 chars).'}, 422
        # userId
        if action == 'add_post':
        ...
        if action == 'add_post':
            return {'msg': f'Key {keymiss} is missing from request.'}, 422
    except (ValueError, AttributeError, TypeError):
        return {'msg': 'Wrong format of the request.'}, 400
    return request_json, 0


def call_external_api(endpoint: str, int_id: int) -> Optional[requests.Response]:
    """ Call external API service to check for resource. """
    url: str = EXTERNAL_API_URL + endpoint + str(int_id)
    try:
        resource = requests.get(
            url, verify=False, timeout=10, headers={'Content-Type': 'application/json'}
        )
        if resource:
            return resource
        return None
